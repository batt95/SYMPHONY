...
int fathom_branch(lp_prob *p)
{
   ...
   while (TRUE)
   {
          ...
          (timeleft = p->tm->par.time_limit - wall_clock(NULL) + p->tm->start_time) <= 0.0)
      {
         ...
         { // send in true for interrupted node
            return (FUNCTION_TERMINATED_NORMALLY);
         }
         else
         {
      ...
      if (p->par.sensitivity_bounds)
      {
         ...
         {
            p->basis_idx = (int *)malloc((lp_data->basis_len) * ISIZE);
         }
         memcpy(p->basis_idx, lp_data->basis_idx, lp_data->basis_len * ISIZE);
         p->basis_len = lp_data->basis_len;
         //-------------------------------------------------------
      ...
      if (p->mip->obj_sense == SYM_MAXIMIZE)
      {
         ...
         {
            PRINT(verbosity, 0, ("The LP value is: %.3f [%i,%i]\n\n", -lp_data->objval + p->mip->obj_offset, termcode, iterd));
         }
      }
      else
      {
            ...
            if (p->node_iter_num > 0 && p->bc_level > 0)
            {
               ...
               if (p->lp_stat.avg_cuts_obj_impr_in_path > 0)
               {
                  p->lp_stat.avg_cuts_obj_impr_in_path =
                      (p->lp_stat.avg_cuts_obj_impr_in_path *
                           (p->lp_stat.num_cut_iters_in_path - 1) +
                       p->lp_data->objval -
                       obj_before_cuts) /
      ...
      comp_times->lp += used_time(&p->tt);
      if (cuts < 0)
      { /* i.e. feasible solution is found */
         if (fathom(p, TRUE, FALSE))
         {
            return (FUNCTION_TERMINATED_NORMALLY);
         }
...
int fathom(lp_prob *p, int primal_feasible, int time_limit_reached)
{
   ...
   case FATHOM__DO_NOT_GENERATE_COLS__DISCARD:
      ...
      send_node_desc(p, termcode == LP_OPT_FEASIBLE ? FEASIBLE_PRUNED : DISCARDED_NODE);
      return (TRUE);

   case FATHOM__DO_NOT_GENERATE_COLS__SEND:
      PRINT(p->par.verbosity, 1, ("Sending node for pricing\n\n"));
      send_node_desc(p, primal_feasible ? OVER_UB_HOLD_FOR_NEXT_PHASE : INFEASIBLE_HOLD_FOR_NEXT_PHASE);
      return (TRUE);
   ...
   case FATHOM__GENERATE_COLS__RESOLVE:
          ...
          termcode == LP_D_OBJLIM || termcode == LP_OPT_FEASIBLE)
      {
         ...
         send_node_desc(p, termcode == LP_OPT_FEASIBLE ? FEASIBLE_PRUNED : OVER_UB_PRUNED);
         free_col_set(&new_cols);
         return (TRUE);
      }
...
int update_cut_parameters(lp_prob *p)
{
   ...
   if (data_par->generate_cgl_probing_cuts == GENERATE_DEFAULT)
   {
      ...
      if (data_par->use_chain_strategy)
      {
         if (p->bc_level > 0 && p->tm->lp_stat.probing_calls + p->lp_stat.probing_calls > 100 &&
   ...
   if (data_par->generate_cgl_twomir_cuts == GENERATE_DEFAULT)
   {
      ...
      if (data_par->use_chain_strategy)
      {
         if (p->bc_level > 0 && p->tm->lp_stat.twomir_calls + p->lp_stat.twomir_calls > 50 &&
   ...
   if (data_par->generate_cgl_flowcover_cuts == GENERATE_DEFAULT)
   {
      ...
      if (data_par->use_chain_strategy)
      {
         if (p->bc_level > 0 && p->tm->lp_stat.flowcover_calls + p->lp_stat.flowcover_calls > 50 &&
               ...
               p->par.max_cut_length =
                   MIN(p->mip->mip_inf->max_row_size,
                       (int)(1.0 * p->par.max_cut_length *
                             p->mip->mip_inf->max_row_size / 500.0) +
                           row_den);
            }
            else
            {
   ...
   case CGL_PROBING_GENERATOR:
   {
      ...
      if (data_par->use_chain_strategy)
      {
            ...
            if (p->iter_num < 2)
            {
               ...
               if (p->mip->mip_inf)
               {
                  p->par.cgl.probing_root_max_look =
                      (int)((1e5 / p->mip->nz) *
                            (5e4 / p->mip->mip_inf->max_row_size)) +
                      1;
                     ...
                     if (p->mip->mip_inf->sos_bin_row_ratio > 0.05)
                     {
                        p->par.cgl.probing_root_max_look =
                            (int)(p->par.cgl.probing_root_max_look /
                                  (200.0 * p->mip->mip_inf->sos_bin_row_ratio)) +
                            1;
                     }
                  }
         ...
         else
         {
            ...
            probing->setMaxLookRoot(MAX(11, (int)(p->par.cgl.probing_root_max_look) / 2 + 10));

            if (p->par.cgl.probing_is_expensive)
            {
               probing->setMaxLookRoot(MAX(5, (int)(p->par.cgl.probing_root_max_look) / 5 + 1));
            }
         }
      }
      else
                           ...
                           int *num_cuts, int *bound_changes, OsiCuts *cutlist, int send_to_pool)
{
   ...
   for (i = 0; i < num_row_cuts; i++)
   {
      ...
      for (int el_num = 0; el_num < num_elements; el_num++)
      {
         ...
         if (!(lp_data->vars[indices[el_num]]->is_int))
            is_int = FALSE;
      ...
      // j++;
#ifdef SYM_COMPILE_IN_LP
      if (p->bc_index < 1 && p->mip->mip_inf && (generator == CGL_PROBING_GENERATOR || generator == CGL_CLIQUE_GENERATOR || generator == CGL_KNAPSACK_GENERATOR))
      {
...
#ifdef SYM_COMPILE_IN_LP
      if (p->bc_level < 1 && p->mip->mip_inf && (generator == CGL_PROBING_GENERATOR || generator == CGL_CLIQUE_GENERATOR || generator == CGL_KNAPSACK_GENERATOR))
      {
