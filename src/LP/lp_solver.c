                ...
                char sense, double lb, double ub)
{
   ...
   {
   case 'E':
      change_lbub(lp_data, col_ind, lb, ub);
      break;
   case 'R':
      change_lbub(lp_data, col_ind, lb, ub);
      break;
   case 'G':
      change_lb(lp_data, col_ind, lb);
      break;
   case 'L':
      change_ub(lp_data, col_ind, ub);
      break;
   default: /*This should never happen ... */
      osllib_status = -1;
...
void constrain_row_set(LPdata *lp_data, int length, int *index)
{
   ...
   for (i = length - 1; i >= 0; i--)
   {
      ...
      case 'R':
         ...
         {
            ub[j] = cut->rhs;
            lb[j] = ub[j] - lp_data->mip->rngval[j];
         }
         else
         {
            ub[j] = cut->rhs;
...
int get_proof_of_infeas(LPdata *lp_data, int *infind)
{
   ...
   if (cpx_status)
      return (0);
   if (jdiv < 0)
   { /* the diverging variable is a slack/range */
      if (lp_data->slacks)
         return (lp_data->slacks[idiv] < 0 ? LOWER_THAN_LB : HIGHER_THAN_UB);
   }
   else
   { /* the diverging variable is structural */
...
void get_slacks(LPdata *lp_data)
{
   ...
   for (i = m - 1; i >= 0; i--)
   {
      if (rows[i].free)
      {
         ...
         {
         case 'E':
            slacks[i] += rows[i].cut->rhs - SYM_INFINITY;
            break;
         case 'L':
            slacks[i] += rows[i].cut->rhs - SYM_INFINITY;
            break;
...
#if defined(__OSI_CPLEX__) || defined(__OSI_OSL__) || defined(__OSI_CLP__) || defined(__OSI_XPRESS__) || defined(__OSI_SOPLEX__) || defined(__OSI_VOL__) || defined(__OSI_DYLP__) || defined(__OSI_GLPK__)
...
void get_dual_ray(LPdata *lp_data)
{
   ...
   double *ray;
   double *origRayCLP = (double *)malloc(DSIZE * lp_data->m);
   int i;
   ...
   if (raysReturned && vRays[0]){
      ray = vRays[0];
      memcpy(origRayCLP, vRays[0], DSIZE * lp_data->m);
      ...
      rayA = (double *)calloc(lp_data->n, DSIZE);
   ...
   if (!ray_is_ok)
   {
      ...
      */
      ...
      int *index = new int[CoinMax(lp_data->n, lp_data->m)];
      int i;
      for (i = CoinMax(lp_data->n, lp_data->m) - 1; i >= 0; --i) {
         index[i] = i;
      }
      double *obj = new double[CoinMax(lp_data->n, 2 * lp_data->m)];
      CoinFillN(obj, lp_data->n, 0.0);
      solver.setObjCoeffSet(index, index + lp_data->n, obj);

      double *clb = new double[2 * lp_data->m];
      double *cub = new double[2 * lp_data->m];

      const double plusone = 1.0;
      const double minusone = -1.0;
      const char *sense = lp_data->si->getRowSense();

      const CoinPackedVectorBase **cols = new const CoinPackedVectorBase *[lp_data->m];
      int newcols = 0;
      for (i = 0; i < lp_data->m; ++i) {
         switch (sense[i]) {
         case 'L':
...
int copy_lp_data(LPdata *lp_data, LPdata *new_data)
{
   ...
   // feb223
   new_data->basis_idx = lp_data->basis_idx;
...
/*===========================================================================*/
int check_cuts(OsiCuts &cutlist, LPdata *lp_data, int bc_level, int *num_cuts, cut_data ***cuts, char send_to_pool, int *bnd_changes,
